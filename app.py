...
# --- 2단계: 이탈의 임계점 분석 ---
    ...
    with tab1:
        ...
        - **Deep Consideration(28-48초)**: 미결정 포기. 콘텐츠에 대한 기대는 유지, 다음 세션 재시도 가능성 높음(회복 가능성 높음)
        - **Decision Fatigue(48초 이상)**: 피로 기반 포기. 일부 세션은 ‘진성 포기자’로 전이되며 장기 이탈 위험이 급격히 증가     
        """)
        st.markdown("---")
    ...
    with tab2:
        ...
        st.markdown("""
        - **무클릭 종료**: 즉각적인 이탈 신호는 아님
        - **Deep Consideration(28-48초)**: 좌절감+기대 붕괴. 이탈 위험이 실제로 ‘행동으로 처음 드러나는 지점'
        - **Decision Fatigue(48초 이상)**: 이미 관여도 높은 유저만 남아 있음. ‘즉시 이탈’이 아니라 ‘장기 전이 위험’ 상태
        """)
        st.markdown("---")
