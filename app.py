...
# --- 2단계: 이탈의 임계점 분석 ---
    ...
    with tab1:
        ...
        # 컬러 코딩: Standard(Gray), Deep(Red), Fatigue(Salmon)
        ...
        fig3.update_layout(showlegend=False, yaxis_range=[12.5, 17], xaxis_title="Group", yaxis_title="Churn Rate (%)",
                           margin=dict(t=100))
        
        # 평균선 추가
        fig3.add_hline(y=14.73, line_dash="dash", line_color="blue", annotation_text="Overall Average (14.73%)", annotation_position="top right")
    ...
    with tab2:
        ...
        fig_abandon = px.bar(df_p2, x='Group', y='Churn_in_Abandon', text='Churn_in_Abandon',
                             title=("** 전체 평균: 무클릭 종료 자체는 ‘즉각적인 이탈 신호’라기보다는 약한 위험 신호→ OTT/콘텐츠 서비스에서 매우 정상적인 구조"
        ...
        fig_abandon.update_layout(showlegend=False, yaxis_range=[13, 17], xaxis_title="Group", yaxis_title="Churn Rate (%)",
                                  margin=dict(t=180))
        
        # 평균선 추가 (14.94%)
        fig_abandon.add_hline(y=14.94, line_dash="dash", line_color="blue", annotation_text="Overall Average (14.94%)", annotation_position="top right")
